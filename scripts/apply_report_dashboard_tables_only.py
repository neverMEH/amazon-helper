#!/usr/bin/env python3
"""
Apply only the missing parts of the collection report dashboard migration
Uses Supabase client to execute individual SQL statements
"""
import os
import sys
...
def execute_sql_via_api(sql: str) -> bool:
    ...
    project_ref = url.split('.')[0].replace('https://', '')
    
    # Use the SQL endpoint (this is a workaround since Python client doesn't expose SQL execution)
    # We'll create the tables using individual operations instead
    return False  # This approach won't work without Management API token


def apply_missing_migration():
    """Apply only the missing parts of the migration"""
    client = get_supabase_client()
    
    print("🚀 Applying Missing Report Dashboard Schema Components")
    print("=" * 60)
    ...
    print("\n📋 The following SQL needs to be executed in the Supabase SQL Editor:")
    print("=" * 60)
    
    sql_statements = []
    
    # 1. Create collection_report_configs table
...
CREATE TABLE IF NOT EXISTS public.collection_report_configs (
    ...
    # 2. Create collection_report_snapshots table
    sql_statements.append("""
...
GRANT SELECT, INSERT, UPDATE, DELETE ON collection_report_snapshots TO authenticated;""")
    ...
    if supabase_url:
        ...
        print(f"\n📎 Click here to open SQL Editor: {dashboard_url}")
    
    print("\n📋 Instructions:")
    print("1. Copy all the SQL above")
    print("2. Open the Supabase SQL Editor using the link")
    print("3. Paste and execute the SQL")
    print("4. Run 'python3 scripts/apply_collection_report_dashboard_migration.py --verify' to confirm")
    
    # Save to a file for convenience
    output_file = Path(__file__).parent / 'migrations' / 'apply_missing_report_dashboard.sql'
...
def main():
    """Main entry point"""
    return apply_missing_migration()
